...
public class DbReservations {
    ...
    public int createReservation(Reservation reservation,int pId) throws SQLException {
        String query = "IF ((\n" +
                ...
                "where (fromDate < ?\n" +
                "      or toDate > ?) AND Reservations.busID = ?) as x)\n" +
                "  <1)\n" +
                "    BEGIN\n" +
    ...
    private PreparedStatement getPrivateFields(Reservation reservation,int pId) throws SQLException{
        ...
        preparedStatement.setInt(3, reservation.getBus().getId());

        preparedStatement.setDate(4, convertUtilToSql(reservation.getFrom()));
        preparedStatement.setDate(5, convertUtilToSql(reservation.getTo()));
        preparedStatement.setDouble(6, reservation.getPrice());
        preparedStatement.setInt(7, reservation.getBus().getId());
